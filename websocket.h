...
size_t make_ws_frame(char *buffer, const char *msg_buffer)
{
    ...
    /*** YOUR CODE HERE ***/
    buffer[0] |= (1<< 7);   // setting fin bit
    buffer[0] |= (1<< 0);   // setting opcode bit


    // masking disabled (only required for client)
    // set payload length (7 bits)
    /*** YOUR CODE HERE ***/
    buffer[1] &=~(1<< 7);           // clearing the mask field
    buffer[1] |= (msg_len<< 0);        // setting the length of the message in the payload_len field.
    // copy the message
    /*** YOUR CODE HERE ***/

    strcat(buffer,msg_buffer);         // concanating header and message.
...
int parse_ws_frame(char *msg_buffer, const char *buffer)
{
    ...
    memset(msg_buffer, 0, sizeof(msg_buffer));   // first we need to clean the msg_buffer since some data will be reserved after writing from the server data if client payload legth is smaller.
    //  printf("fin=%d \n,opcode=%d \n, mask=%d \n", fin,opcode,mask); for debugging purposes to see what comes
    ...
    /*** YOUR CODE HERE ***/
if (fin==128 && opcode==0x01){    // if fin==1 and opcode ==0x01 then take frame and look at whether it is masked or not.
        if(mask!=128){
            for(int i=0; i<payload_len; i++)
            {
                msg_buffer[i]=buffer[i+2];    // i+2 since first 2 byte is header
            }
